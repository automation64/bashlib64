...
@test "bl64_fmt_url_encode: printable_non_alphanumeric" {
  output="$(cat "$printable_non_alphanumeric" | bl64_fmt_url_encode)"
  sample="$(<$url_chars_ascii_non_alphanumeric)"
  assert_equal "$output" "$sample"
}

@test "bl64_fmt_url_encode: printable_alphanumeric" {
  output="$(cat "$printable_alphanumeric" | bl64_fmt_url_encode)"
  sample="$(<$url_chars_ascii_alphanumeric)"
  assert_equal "$output" "$sample"
}

@test "bl64_fmt_url_encode: printable_all" {
  output="$(cat "$printable_all" | bl64_fmt_url_encode)"
  sample="$(<$url_chars_ascii_all)"
  assert_equal "$output" "$sample"
}

@test "bl64_fmt_url_encode: url_chars_ascii_reserved" {
  output="$(cat "$url_chars_ascii_url_reserved" | bl64_fmt_url_encode)"
